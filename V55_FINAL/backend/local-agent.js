...
async function sendTelegram(title,body,options={}){
  if(!telegramConfigured()){
    await db.collection('system').doc('agent').set({
      ...
      telegramLastError:'Bot Token 또는 Chat ID가 없습니다.',
      telegramCheckedAt:new Date().toISOString()
    },{merge:true}).catch(()=>{});
    return {enabled:false,sent:0,failed:1,error:'설정 없음'};
  }
  ...
  for(let attempt=1;attempt<=attempts;attempt+=1){
    try{
      ...
      if(attempt<attempts){
        ...
        console.log(`텔레그램 재시도 ${attempt}/${attempts}`);
...
async function sendPush(orders){
  ...
  for(const order of orders){
    if(order.eventType!=='order') continue;

    const result=await sendTelegram(
      `${telegramMarketIcon('쿠팡')} 쿠팡 주문 발견 · ${order.statusLabel||'신규주문'}`,
  ...
  if(orders.length){
    console.log(`쿠팡 텔레그램 완료: 성공 ${sent}, 실패 ${failed}`);
  }

  return {devices:0,sent,failed,channel:'telegram'};
}
...
function monthStartMinutes(){
  ...
  return Math.ceil((now.getTime()-start.getTime())/60000)+60;
}
  ...
  try{
    return await Promise.race([
      ...
      new Promise((_,reject)=>{
        timer=setTimeout(
          ()=>reject(new Error(`${label} ${Math.round(ms/1000)}초 응답시간 초과`)),
          ms
        );
      })
    ]);
...
async function fastSync(source='interval'){
  ...
  const result=await withTimeout(
    ...
    pollCoupangStatuses(db,coupang(),{
      ...
      maxPages:reconcile?12:2
    }),
    reconcile?120000:45000
  );
...
async function sendMarketplacePush(orders,marketName){
  ...
  for(const order of orders){
    ...
    const result=await sendTelegram(
      `${telegramMarketIcon(marketName)} ${marketName} 주문 발견 · ${order.statusLabel||'신규주문'}`,
      telegramOrderBody(order)
    );
  ...
  if(orders.length){
    console.log(`${marketName} 텔레그램 완료: 성공 ${sent}, 실패 ${failed}`);
  }
...
async function runFastSync(){
  ...
  try{
    const fast=await fastSync('interval');
...
async function run(source){
  ...
  try{
    try{
      const fast=await fastSync(source);
      summary.coupang=fast;
      await saveIntegration(fast,null);
      console.log(`쿠팡 완료: 발견 ${fast.found}, 신규 ${fast.created}, 상태변경 ${fast.statusChanged}`);
    }catch(error){
      summary.coupang={error:error.message};
      console.error('쿠팡 실패:',error.message);
    }
    ...
    try{
      summary.claims=
        reconcile||source==='immediate'
          ?await withTimeout('쿠팡 CS 전체조회',syncAllClaimTypes(),120000)
          :source==='interval'
            ?[await withTimeout('쿠팡 CS 순환조회',syncOneClaimType(),45000)]
            :[];
    }catch(error){
    ...
    console.error(`${label} 전체 오류:`,error.message);
