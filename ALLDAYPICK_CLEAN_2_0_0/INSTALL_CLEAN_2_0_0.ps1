...
$backup = Join-Path $target "backup-before-clean-v2.0.0-$stamp"

if(!(Test-Path (Join-Path $target ".git"))){
  Write-Host "ERROR: ALLDAYPICK_CLEAN_2_0_0 must be directly inside the project folder." -ForegroundColor Red
...
@(
  ...
  "manifest.json","icon.svg","firestore.rules.txt",".gitignore"
) | ForEach-Object {
...
Write-Host ""
Write-Host "CLEAN v2.0.0 installed." -ForegroundColor Green
