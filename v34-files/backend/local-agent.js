...
async function updateElevenstConnectionStatus(){
  try{
    const config=elevenstConfigFromEnv(process.env);
    const result=await checkElevenstConfiguration(db,config);

    if(result.configured){
      console.log('11번가 API 키 등록 확인 · 주문 API 연결 대기');
    }else{
      console.log('11번가 Open API 키가 아직 등록되지 않았습니다.');
    }
  }catch(error){
    const message=error instanceof Error?error.message:String(error);
    console.error('11번가 연결상태 확인 실패:',message);
...
async function run(source){
  ...
  try{
    ...
    await updateElevenstConnectionStatus();
