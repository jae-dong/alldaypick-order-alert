...
function mapElevenStatus(row = {}) {
  ...
  if (value.includes('DELIVERED') || value.includes('배송완료') || value.includes('구매확정')) {
  ...
  for (let i = 0; i < orderNos.length; i += 20) {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        const invoiceNumber = first(row,['invoiceNo','trackingNumber','dlvNo']);
        const deliveryCompanyName = first(row,['dlvCpnNm','deliveryCompanyName','deliveryCompany']);
        ...
        for (const order of matches) {
          ...
          await db.collection('orders').doc(order.id).set({
            ...
            deliveryCompanyName:deliveryCompanyName || order.deliveryCompanyName || '',
            syncedAt:new Date().toISOString(),
            updatedAt:admin.firestore.FieldValue.serverTimestamp()
          }, { merge:true });
