...
const refreshSelection=H.statusRefreshDocuments([
  ...
  {id:'inactive-done',eventType:'order',status:'delivered',activeState:false,orderNo:'300'},
  {id:'active-claim',eventType:'return',activeState:true,orderNo:'400'}
]);
assert.deepEqual(refreshSelection.normalExisting.map(item=>item.id),['inactive-pending']);
