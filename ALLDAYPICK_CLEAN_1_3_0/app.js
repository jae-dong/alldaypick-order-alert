const APP_VERSION='CLEAN v1.3.0';
...
function statsGroupsForPeriod(period){const all=latestNormalGroups();if(period==='all')return all;const days=Math.max(1,Number(period)||7),cutoff=Date.now()-days*86400000;return all.filter(g=>g.date?.getTime?.()>=cutoff);}
...
function canonicalDate(order){
  const values=[
    order?.orderDate,order?.orderAt,order?.orderedAt,
    order?.paymentDate,order?.paymentAt,
    order?.datetime,order?.createdAt
  ];
...
function canonicalDay(order){
  const date=canonicalDate(order);
...
function canonicalKey(order){
  return [
    normalizedText(order?.market||order?.source),
    normalizedText(
      order?.orderNo||order?.orderId||
      order?.shipmentBoxId||order?.deliveryNo||order?.id
    )
  ].join('|');
}

function canonicalAmount(order){
  for(const value of [
    order?.orderTotalAmount,
    order?.totalAmount,
    order?.paymentAmount,
...
function canonicalNormalGroups(){
  const groups=new Map();
  ...
  latestOrderLines()
    .filter(order=>{
      ...
      return (
        isOrderEvent(order) &&
        marketIncludedOrder(order) &&
        !['cancel','return','exchange','inquiry'].includes(key)
      );
    })
    .forEach(order=>{
      const key=canonicalKey(order);
      ...
      if(!groups.has(key)){
        groups.set(key,{
          ...
          market:order.market||order.source||'기타',
          orderNo:order.orderNo||order.orderId||'',
          date:canonicalDate(order),
          day:canonicalDay(order),
          lines:[],
          qty:0,
          lineAmount:0,
          explicitTotal:0
      ...
      if(Number.isFinite(explicit)&&explicit>0){
        ...
        group.lineAmount+=canonicalAmount(order);
      }
    });
...
function canonicalTodayGroups(){
  const today=todayKey();
  return canonicalNormalGroups().filter(group=>group.day===today);
}

function canonicalMonthGroups(){
  const month=monthKey();
  return canonicalNormalGroups().filter(
    group=>String(group.day||'').slice(0,7)===month
  );
}

function terminalText(order){
  return [
    order?.sourceStatus,order?.status,order?.statusLabel,
    order?.claimStatus,order?.processingStatus,
    order?.resultStatus,order?.receiptStatus,order?.exchangeStatus
  ].filter(Boolean).join(' ').toUpperCase();
}

function completedWork(order){
  if(isProcessed(order)) return true;

  const text=terminalText(order);
...
function canonicalUnresolvedItems(){
  const ordersOnly=latestOrderLines().filter(order=>
    isOrderEvent(order) &&
    marketIncludedOrder(order) &&
    ['new','shipping_wait'].includes(statusKey(order)) &&
    !completedWork(order)
  ...
  const claims=latestClaimDocuments().filter(order=>
    marketIncludedOrder(order) &&
    ['cancel','return','exchange','inquiry'].includes(statusKey(order)) &&
    !completedWork(order)
  );

  return [...ordersOnly,...claims];
}

function canonicalUnresolvedCounts(){
  const counts={
    new:0,shipping_wait:0,cancel:0,
    return:0,exchange:0,inquiry:0
  };
...
function canonicalUnresolvedByMarket(){
  ...
  MARKETS.forEach(([,name])=>{
    result[name]={
      new:0,shipping_wait:0,cancel:0,
      return:0,exchange:0,inquiry:0,orderAmount:0
    };
  });
  ...
  canonicalUnresolvedItems().forEach(item=>{
    ...
    if(!result[market]||!(key in result[market])) return;

    result[market][key]+=1;

    if(['new','shipping_wait'].includes(key)){
      result[market].orderAmount+=canonicalAmount(item);
    }
  });
...
function latestStateTimestamp(order){
  for(const value of [order?.sourceUpdatedAt,order?.lastChangedAt,order?.statusChangedAt,order?.updatedAt,order?.createdAt,order?.datetime]){
    if(!value) continue;
    if(value?.toDate){const t=value.toDate().getTime();if(Number.isFinite(t))return t;}
    const t=new Date(value).getTime();if(Number.isFinite(t))return t;
  }
  return 0;
}
function latestStateKey(order){
  return [normalizedText(order?.market||order?.source),normalizedText(order?.orderNo||order?.orderId||order?.shipmentBoxId||order?.deliveryNo||order?.id)].join('|');
}
function latestStatePerOrder(){
  const map=new Map();
  orders.filter(marketIncludedOrder).forEach(order=>{
    const key=latestStateKey(order), current=map.get(key);
    if(!current||latestStateTimestamp(order)>=latestStateTimestamp(current))map.set(key,order);
  });
  return [...map.values()];
}
function latestStatusText(order){return [order?.sourceStatus,order?.status,order?.statusLabel,order?.claimStatus,order?.processingStatus,order?.resultStatus,order?.receiptStatus,order?.exchangeStatus].filter(Boolean).join(' ').toUpperCase();}
function latestStateCompleted(order){
  if(isProcessed(order))return true;
  const text=latestStatusText(order);
...
function latestNormalGroups(){
  return latestStatePerOrder().filter(o=>String(o?.eventType||'order').toLowerCase()==='order'&&!['cancel','return','exchange','inquiry'].includes(statusKey(o))).map(o=>({key:latestStateKey(o),market:o.market||o.source||'기타',orderNo:o.orderNo||o.orderId||'',date:canonicalDate(o),day:canonicalDay(o),lines:[o],qty:Number(o.qty||1),amount:canonicalAmount(o)}));
}
function latestTodayGroups(){const t=todayKey();return latestNormalGroups().filter(g=>g.day===t);}
function latestMonthGroups(){const m=monthKey();return latestNormalGroups().filter(g=>String(g.day||'').slice(0,7)===m);}
function latestUnresolvedItems(){
  return latestStatePerOrder().filter(o=>{
    const k=statusKey(o);
    return ['new','shipping_wait','cancel','return','exchange','inquiry'].includes(k)&&!latestStateCompleted(o);
  });
}
function latestUnresolvedCounts(){const c={new:0,shipping_wait:0,cancel:0,return:0,exchange:0,inquiry:0};latestUnresolvedItems().forEach(o=>{const k=statusKey(o);if(k in c)c[k]++});return c;}
function latestUnresolvedByMarket(){
  const r={};MARKETS.forEach(([,n])=>r[n]={new:0,shipping_wait:0,cancel:0,return:0,exchange:0,inquiry:0,orderAmount:0});
  latestUnresolvedItems().forEach(o=>{const m=o.market,k=statusKey(o);if(!r[m])return;if(k in r[m])r[m][k]++;if(['new','shipping_wait'].includes(k))r[m].orderAmount+=canonicalAmount(o)});return r;
}


function correctedPendingByMarket(){return latestUnresolvedByMarket();}
...
function correctedTodayTotals(){const g=latestTodayGroups();return {count:g.length,sales:g.reduce((s,x)=>s+Number(x.amount||0),0)};}

function correctedMonthTotals(){const g=latestMonthGroups();return {count:g.length,sales:g.reduce((s,x)=>s+Number(x.amount||0),0)};}
...
function renderStatus(){const counts=latestUnresolvedCounts(),grid=$('statusGrid');if(!grid)return;grid.innerHTML=STATUS_ITEMS.map(([key,label])=>`<button class="alert-card ${activeStatus===key?'active':''}" data-key="${key}"><span>${label}</span><strong>${Number(counts[key]||0)}</strong></button>`).join('');grid.querySelectorAll('button').forEach(b=>b.onclick=()=>{activeStatus=activeStatus===b.dataset.key?'':b.dataset.key;currentPage=1;showOrdersTab();render();});const d=$('dedupeInfo');if(d)d.textContent='주문번호별 최신 상태 기준 · 완료되면 자동 제외';$('statusUpdated').textContent='최근 갱신 '+new Date().toLocaleTimeString('ko-KR',{hour:'2-digit',minute:'2-digit'});}
function renderMarkets(){const pending=latestUnresolvedByMarket();$('marketBody').innerHTML=MARKETS.map(([key,name])=>{const d=pending[name]||{},connected=Boolean(integrations[key]?.connected),count=Number(d.new||0)+Number(d.shipping_wait||0);return `<tr class="market-row ${activeMarket===name?'selected':''}" data-market="${name}"><td><span class="market-name"><span class="market-dot ${connected?'ok':''}"></span>${name}</span></td><td class="order-sales-cell"><strong>${count}</strong><small>${fmt(d.orderAmount||0)}</small></td><td>${Number(d.new||0)}</td><td>${Number(d.shipping_wait||0)}</td><td>${Number(d.cancel||0)}</td><td>${Number(d.return||0)}</td><td>${Number(d.exchange||0)}</td></tr>`}).join('');$('marketBody').querySelectorAll('tr').forEach(row=>row.onclick=()=>{activeMarket=activeMarket===row.dataset.market?'':row.dataset.market;currentPage=1;showOrdersTab();render();$('ordersPanel').scrollIntoView({behavior:'smooth',block:'start'});});$('marketUpdated').textContent='주문번호별 최신 상태 기준 · '+new Date().toLocaleTimeString('ko-KR',{hour:'2-digit',minute:'2-digit'});}
function filteredOrders(){const q=$('searchInput').value.trim().toLowerCase(),market=$('marketFilter').value,read=$('readFilter').value;return latestUnresolvedItems().filter(o=>{const s=statusKey(o);if(activeStatus&&s!==activeStatus)return false;if(activeMarket&&o.market!==activeMarket)return false;if(market&&o.market!==market)return false;const hit=!q||[o.product,o.orderNo,o.buyer,o.phone,o.invoiceNumber,o.workflowNote].some(v=>String(v||'').toLowerCase().includes(q));if(!hit)return false;if(read==='unread'&&!isUnread(o))return false;if(read==='read'&&isUnread(o))return false;return true;}).sort((a,b)=>latestStateTimestamp(b)-latestStateTimestamp(a));}
...
if('serviceWorker' in navigator){
  navigator.serviceWorker.getRegistrations()
    ...
    .finally(()=>navigator.serviceWorker.register('./sw.js?v=clean-v1.3.0',{updateViaCache:'none'}))
    .catch(console.warn);
}
