const APP_VERSION='CLEAN v1.2.0';
...
function statsGroupsForPeriod(period){
  const all=canonicalNormalGroups();
...
function correctedPendingByMarket(){return canonicalUnresolvedByMarket();}
...
function correctedTodayTotals(){
  ...
  return {
    count:groups.length,
    sales:groups.reduce((sum,g)=>sum+Number(g.amount||0),0)
  };
}
...
function renderStatus(){
  const counts=canonicalUnresolvedCounts();
  ...
  if(dedupeInfo){
    dedupeInfo.textContent='현재 실제 미완료 건만 표시 · 완료되면 자동 제외';
  }

  $('statusUpdated').textContent=
    '최근 갱신 '+new Date().toLocaleTimeString(
      'ko-KR',{hour:'2-digit',minute:'2-digit'}
    );
}
function renderMarkets(){
  const pending=canonicalUnresolvedByMarket();

  $('marketBody').innerHTML=MARKETS.map(([key,name])=>{
    const data=pending[name]||{};
    const connected=Boolean(integrations[key]?.connected);
    const count=Number(data.new||0)+Number(data.shipping_wait||0);

    return `
      <tr class="market-row ${activeMarket===name?'selected':''}" data-market="${name}">
        <td><span class="market-name"><span class="market-dot ${connected?'ok':''}"></span>${name}</span></td>
        <td class="order-sales-cell"><strong>${count}</strong><small>${fmt(data.orderAmount||0)}</small></td>
...
function filteredOrders(){
  ...
  return canonicalUnresolvedItems().filter(order=>{
    ...
    const hit=!q||[
      order.product,order.orderNo,order.buyer,
      order.phone,order.invoiceNumber,order.workflowNote
...
if('serviceWorker' in navigator){
  navigator.serviceWorker.getRegistrations()
    ...
    .finally(()=>navigator.serviceWorker.register('./sw.js?v=clean-v1.2.0',{updateViaCache:'none'}))
    .catch(console.warn);
}
